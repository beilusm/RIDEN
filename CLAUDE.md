...
- **Phase C — CH340 串口自动扫描 + 400ms 自动重试**：APPLIED（用户明确请求，override CLAUDE.md "不新增业务功能" 禁令。`lib/services/serial_port_scanner.dart` 纯 Dart 类 + `lib/services/serial_port_enumerator.dart` FFI isolate 入口；`SerialModbusService.connect` 新增 `scannerFactory` 构造参数，port==null 时用 `enumerateUsbPortsViaIsolate`（`Isolate.run` + libserialport FFI `sp_get_port_usb_vid_pid`）按 VID=0x1A86 / PID=0x7523 选端口；找不到 CH340 抛 `SerialPortScanException`，不启动 polling。手动端口路径保留 — explicit port bypass scanner。**connect() reorder: scan 在 spawn 之前** — 失败 cycle 不 spawn worker（~50ms→~30ms/次）。**PowerSupplyProvider 自动重试**：`_autoScanTimer` 400ms `Timer.periodic` + `startAutoScan()` / `stopAutoScan()` / `isAutoScanning` getter；app boot (app.dart initState) 自动 `startAutoScan`；tick 走 `_connected || _connecting` guard，connected 时 ticks no-op 但 timer 不断；user `disconnect()` 调 `stopAutoScan` 不打脸；worker crash (P1-2) keeps timer alive → 下次 400ms tick 自动 relight connect 无需用户介入；`dispose()` 兜底取消。不修改 ModbusScheduler / ModbusWorker / quickSwitch / polling 架构 / 数据模型 / 寄存器定义。`flutter analyze` 21 issues / 0 新增；`flutter test` 24→48 PASS（24 baseline + 12 scanner pure + 4 connect-integration + 8 auto-scan retry via fakeAsync））
